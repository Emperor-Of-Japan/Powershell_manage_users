...
#
#
#paypal: https://paypal.me/pools/c/8A9t8tPZrV
#
#
#
#
...
<Window xmlns="http://schemas.microsoft.com/winfx/2006/xaml/presentation"
    ...
    <Grid>
        ...
        <TabControl HorizontalAlignment="Left" Height="409" VerticalAlignment="Top" Width="421"  TabStripPlacement="Top" Opacity="0.75">
            ...
            <TabItem Header="migrer un/des utilisateur" >
                ...
                <Grid Background="#FFE5E5E5">
                    ...
                    <ComboBox Name="combobox_migr_user" HorizontalAlignment="Left" Height="22" Margin="10,50,0,0" VerticalAlignment="Top" Width="120"/>
                    <Label Content="Groupe:" HorizontalAlignment="Left" Margin="285,20,0,0" VerticalAlignment="Top"/>
                </Grid>
            </TabItem>
        ...
        <ProgressBar Name="progress_bar" HorizontalAlignment="Left" Height="23" Margin="662,7,0,0" VerticalAlignment="Top" Width="120" Background="#FF387182"/>
        <CheckBox Name="tls_blox" Content="Enable SSL/TLS" HorizontalAlignment="Left" Margin="662,7,0,0" VerticalAlignment="Top"/>
    </Grid>
</Window>
...
$combo_recherche_migr=$Form.FindName("combobox_migr_user")
#listbox
...
$listbox_migr_enlever_grp=$Form.FindName("lisbox_migr_ancien_grp")
#autre
$tls_box=$Form.FindName("tls_blox")
#-------------------------------------------------------------------------------------------------------------------------------------------
#-------------------------------------------------------------------------------------------------------------------------------------------
#-------------------------------------------------------------------------------------------------------------------------------------------
#méthode 
...
function Refresh {
    ...
    $path2=$uo_combo.SelectedItem.ToString()
             if($tls_box=$chkChange.Checked)
    {
        Invoke-command -ComputerName $textbox_srv  -credential $Cred -ArgumentList $path2 -UseSSL -ScriptBlock {param($path2)Get-ADUser -Filter * -Searchbase $path2 | Select-Object SamAccountName}
    }
    else{
...
function Recherche{
    ...
    $path3=$uo_combo.SelectedItem.ToString()
                 if($tls_box=$chkChange.Checked)
    {
         $recherche=Invoke-command -ComputerName $textbox_srv  -credential $Cred -ArgumentList $path3 -UseSSL -ScriptBlock {param($path3) Get-ADUser -Filter * -Searchbase $path3 | Select-Object SamAccountName}
    }
    else{
...
$textbox_nom_serveur=$Form.FindName("textbox_nom_srv")
 ...
 $Form.FindName("button_connexion").Add_Click( {
    ...
    if($tls_box=$chkChange.Checked)
    {
        $test=Invoke-command -ComputerName $textbox_srv -credential $Cred -UseSSL -ScriptBlock {Hostname}
    }
    else{
    ...
    [void] $uo_combo.Items.Clear()
     #remplie la combo du choix des UO
         if($tls_box=$chkChange.Checked)
    {
        $organizationUnitName=Invoke-command -ComputerName $textbox_srv -credential $Cred -UseSSL -ScriptBlock {(Get-ADOrganizationalUnit -Filter *).DistinguishedName}
    }
    else{
    ...
    #Information sur l'environment
             if($tls_box=$chkChange.Checked)
    {
        $domain= Invoke-command -ComputerName $textbox_srv  -credential $Cred -UseSSL -ScriptBlock {Get-ADDomain -Current LocalComputer} #nom de l'ordinateur
    $nom_computer= Invoke-command -ComputerName $textbox_srv  -credential $Cred -UseSSL -ScriptBlock {(Get-ADDomainController -Filter * | Select Name).name} #nom netbios du domaine
    }
    else{
    ...
    If ($uo_combo.text -ne $null) {
        ...
        [void] $combo_recherche_migr.Items.Clear()
        
         $uo_selected=$uo_combo.SelectedItem.ToString()
                  if($tls_box=$chkChange.Checked)
                    {
                        $script:ListeGr = Invoke-command -ComputerName $textbox_srv -credential $Cred -ArgumentList $uo_selected -UseSSL -ScriptBlock {param($uo_selected) (Get-ADGroup -Properties * -Filter * -SearchBase "$uo_selected").name}
                            Invoke-command -ComputerName $textbox_srv -credential $Cred -UseSSL -ScriptBlock {ADD-content -path C:/log.txt -value "----------------------------------------------------------------------------------------------------`r`nscript lancé le $using:date à $using:hour par $using:userName"}
                     }
    else{
        ...
        $combo_recherche_migr.Items.Add($Usergr)
        }
        }
    Refresh
    Recherche
...
#-------------------------------------------------------------------------------------------------------------------------------------------
#-------------------------------------------------------------------------------------------------------------------------------------------
#button Refresh recharge des éléments en cas de bug/de problème
 $Form.FindName("refresh_users").Add_Click( {
    Refresh
    Recherche
    Refresh_listbox
    $uo_combo.Add_SelectionChanged($uo_comboIndexChanged)#}
#changer les groupes des combobox après chaque changement d'UO
    ...
    if($etu_prenom.Length -lt 3 -or $etu_nom.Length -lt 3-or $nam_grp -eq $null)#si une DATA n'est pas remplis on ne crée pas l'Etudiant
    {
        ...
        if([bool] (Invoke-command -ComputerName $textbox_srv  -credential $Cred  -ScriptBlock {Get-ADUser -Filter {sAMAccountName -eq  $using:session}}) -eq 1){
            ...
            $password = ConvertTo-SecureString "P@ssword" -AsPlainText -Force #on set le mdp par défault , il devra etre modifié à la première connection 
             if($tls_box=$chkChange.Checked)
                    {
                        Invoke-command -ComputerName $textbox_srv -credential $Cred -UseSSL -ScriptBlock {
            New-ADUser -Name $using:nom_complet -GivenName $using:etu_prenom -DisplayName $using:nom_complet -Surname $using:nom_complet -SamAccountName $using:session -UserPrincipalName $using:session -Enabled $true -AccountPassword $using:password -ChangePasswordAtLogon $True -Path $using:path1 
            Add-ADGroupMember -Identity  $using:nam_grp -Members $using:session -PassThru
            {ADD-content -path C:/log.txt -value "$using:session ajouté à l'AD au groupe $using:groupe"
            
                     }
        else{
            Invoke-command -ComputerName $textbox_srv -credential $Cred -ScriptBlock {
            New-ADUser -Name $using:nom_complet -GivenName $using:etu_prenom -DisplayName $using:nom_complet -Surname $using:nom_complet -SamAccountName $using:session -UserPrincipalName $using:session -Enabled $true -AccountPassword $using:password -ChangePasswordAtLogon $True -Path $using:path1 
            Add-ADGroupMember -Identity  $using:nam_grp -Members $using:session -PassThru
            {ADD-content -path C:/log.txt -value "$using:session ajouté à l'AD au groupe $using:groupe"
            }
...
#button supprimer l'utilisateur
 $Form.FindName("valider_supp").Add_Click( {
  ...
  'Yes' {
        ...
        if($tls_box=$chkChange.Checked)
                    {
                    Invoke-command -ComputerName $textbox_srv  -credential $Cred -ArgumentList $sam -UseSSL -ScriptBlock {
                    param($sam)Remove-ADUser $sam -Confirm:$false
                    ADD-content -path C:/log.txt -value "$using:sam supp de l'AD le $using:date à $using:hour"
                    } #on supp l'étudiant
                    }
                    else 
                    {
        ...
        Invoke-command -ComputerName $textbox_srv  -credential $Cred -ArgumentList $sam -ScriptBlock {
        param($sam)Remove-ADUser $sam -Confirm:$false
        ADD-content -path C:/log.txt -value "$using:sam supp de l'AD le $using:date à $using:hour"
        } #on supp l'étudiant
...
#code du bouton Transferer
 $Form.FindName("valider").Add_Click( {
    ...
    try{$nam_grp=$combo_recherche_migr.SelectedItem.ToString()}catch{""} 
    if ($nom_etu_migr -ne $null -and $nam_new_grp -ne $null){
     foreach($users in $nom_etu_migr){
     if($tls_box=$chkChange.Checked)
                    {
                    Invoke-command -ComputerName "WIN-2F6TT0VV10R"  -credential $Cred -UseSSL -ScriptBlock {
        Remove-ADGroupMember $using:nam_grp -Members $using:nom_etu_migr -Confirm:$false
        Add-ADGroupMember -Identity  $using:nam_new_grp -Members $using:nom_etu_migr -PassThru
        ADD-content -path C:/log.txt -value "$using:nom_etu_migr ajouté dans le groupe $using:nam_new_grp le $using:date à $using:hour"
        }
                    }
                    else{
        Invoke-command -ComputerName "WIN-2F6TT0VV10R"  -credential $Cred -ScriptBlock {
        Remove-ADGroupMember $using:nam_grp -Members $using:nom_etu_migr -Confirm:$false
        Add-ADGroupMember -Identity  $using:nam_new_grp -Members $using:nom_etu_migr -PassThru
        ADD-content -path C:/log.txt -value "$using:nom_etu_migr ajouté dans le groupe $using:nam_new_grp le $using:date à $using:hour"
        }}
...
$combox_migration_1SelectedIndexChanged=
{
   If ($combo_recherche_migr.text -ne $null) {
        [void] $listbox_etu_AD.Items.Clear()
        $nam_grp=$combo_recherche_migr.SelectedItem.ToString() 
        if($tls_box=$chkChange.Checked)
                    {
                    $recup_group=Invoke-command -ComputerName $textbox_srv -credential $Cred -ArgumentList $nam_grp -UseSSL -ScriptBlock {param($nam_grp)Get-ADGroupMember -identity  $nam_grp | select SamAccountName}
                    }
                    else {
        $recup_group=Invoke-command -ComputerName $textbox_srv -credential $Cred -ArgumentList $nam_grp -ScriptBlock {param($nam_grp)Get-ADGroupMember -identity  $nam_grp | select SamAccountName}
        }
        foreach ($users in $recup_group){
        $sam=$users.SamAccountName
             [void] $listbox_etu_AD.Items.Add("$sam")#commentaire
        }
   }
}
################ MUST CREATE BEFORE ASSIGN ################
$combo_recherche_migr.Add_SelectionChanged($combox_migration_1SelectedIndexChanged)
...
$Form.FindName("valider_creation_groupe").Add_Click( { 
    ...
    if($group -ne $Null -and $users -ne $null){ #si un groupe est choisi et un chemin 
        ...
        $session=$nom.ToUpper()
        $path=$uo_combo.SelectedItem.ToString()
                
                 if($tls_box=$chkChange.Checked)
                    {
                    Invoke-command -ComputerName "WIN-2F6TT0VV10R"  -credential $Cred -UseSSL -ScriptBlock {
                New-ADUser -Name $using:nom_complet -GivenName $using:prenom -DisplayName $using:nom_complet  -Surname $using:nom -SamAccountName $using:session -UserPrincipalName $using:session -AccountPassword $using:password -ChangePasswordAtLogon $True -Path $using:path -Enabled $true
                Add-ADGroupMember -Identity  $using:group -Members $using:nom -PassThru
                ADD-content -path C:/log.txt -value "$using:session ajouté à l'AD le $using:date à $using:hour"
                }
                    }
                    else {
                Invoke-command -ComputerName "WIN-2F6TT0VV10R"  -credential $Cred -ScriptBlock {
...
$Form.FindName("validation").Add_Click( { 
    ...
    switch  ($msgBoxInput) {
        ...
        'Yes' {
            ...
            if($tls_box=$chkChange.Checked)
                    {
            try{$users=Invoke-command -ComputerName $textbox_srv -credential $Cred -ArgumentList $myname_supp -UseSSL -ScriptBlock {param($myname_supp)Get-ADGroupMember -identity $myname_supp | select SamAccountName}}catch{""}
            }
            else{
            ...
            foreach ($user in $users){
                ...
                try{Invoke-command -ComputerName $textbox_srv -credential $Cred -ArgumentList $sam -UseSSL -ScriptBlock {
                param($sam)Remove-ADUser $sam -Confirm:$false
                ADD-content -path C:/log.txt -value "$using:sam supp de l'AD le $using:date à $using:hour"}}catch{""}
                    }
                    else{
                $sam=$user.SamAccountName
                try{Invoke-command -ComputerName $textbox_srv -credential $Cred -ArgumentList $sam -ScriptBlock {
                param($sam)Remove-ADUser $sam -Confirm:$false
                ADD-content -path C:/log.txt -value "$using:sam supp de l'AD le $using:date à $using:hour"
...
$Form.FindName("valider_transfert").Add_Click( { 
    ...
    if($old_groupe -eq $Null -or $new_groupe -eq $null)
    {
        ...
        $sam=$user.SamAccountName
                 if($tls_box=$chkChange.Checked)
                    {
                     try{Invoke-command -ComputerName $textbox_srv -credential $Cred -UseSSL -ScriptBlock {
                remove-ADGroupMember -Identity  $using:old_groupe -Member $using:sam -Confirm:$false
                Add-ADGroupMember -Identity  $using:new_groupe -Members $using:sam -PassThru
                ADD-content -path C:/log.txt -value "$using:nom_etu_migr ajouté dans le groupe $using:new_groupe le $using:date à $using:hour"
                }}catch{""} #on retire l'étudiant de l'ancien groupe
                    
                    }
                    else{
                try{Invoke-command -ComputerName $textbox_srv -credential $Cred -ScriptBlock {
                remove-ADGroupMember -Identity  $using:old_groupe -Member $using:sam -Confirm:$false
                Add-ADGroupMember -Identity  $using:new_groupe -Members $using:sam -PassThru
                ADD-content -path C:/log.txt -value "$using:nom_etu_migr ajouté dans le groupe $using:new_groupe le $using:date à $using:hour"
                }}catch{""} #on retire l'étudiant de l'ancien groupe
...
$combox_migration_SelectedIndexChanged=
{
   If ($combox_migration.text -ne $null) {
        ...
        if($tls_box=$chkChange.Checked)
                    {
                    $recup_group=Invoke-command -ComputerName $textbox_srv -credential $Cred -ArgumentList $nam_new_grp -UseSSL -ScriptBlock {param($nam_new_grp)Get-ADGroupMember -identity  $nam_new_grp | select SamAccountName}
                    }
                    else{
...
function Refresh_listbox{
        $Form.Dispatcher.Invoke([Windows.Threading.DispatcherPriority]::Background, [action]{
        #on refresh la listbox de droite du pannel migrer un/des utiisateurs
         [void] $listbox_groupe_new.Items.Clear()
         ...
         foreach ($users in $recup_group){
             ...
             [void] $listbox_groupe_new.Items.Add("$sam")#commentaire
                ...
                #on refresh la listbox de gauche du pannel migrer un/des utiisateurs
         [void] $listbox_etu_AD.Items.Clear()
         try{$nam_new_grp=$combo_recherche_migr.SelectedItem.ToString() 
                    if($tls_box=$chkChange.Checked)
                    {
                    $recup_group=Invoke-command -ComputerName $textbox_srv -credential $Cred -ArgumentList $nam_new_grp -UseSSL -ScriptBlock {param($nam_new_grp) Get-ADGroupMember -identity  $nam_new_grp | select SamAccountName}
                    }
        ...
        try{$nam_old_grp=$combox_migr_grp_etu_left.SelectedItem.ToString()
                     if($tls_box=$chkChange.Checked)
                    {
                    $recup_group=Invoke-command -ComputerName $textbox_srv -credential $Cred -ArgumentList $nam_old_grp -UseSSL -ScriptBlock {param($nam_old_grp)Get-ADGroupMember -identity  $nam_old_grp | select SamAccountName}
                    }
                    else{ 
       ...
       #on refresh la listbox de droite du pannel "migrer tout un groupe"
        ...
        try{$nam_new_grp=$combox_migr_grp_etu_right.SelectedItem.ToString()
                    if($tls_box=$chkChange.Checked)
                    {
                     $recup_group1=Invoke-command -ComputerName $textbox_srv -credential $Cred -ArgumentList $nam_new_grp -UseSSL -ScriptBlock {param($nam_new_grp)Get-ADGroupMember -identity  $nam_new_grp | select SamAccountName}
                    }
                    else{ 
        ...
        try{$nam_grp=$combox_supp_grp_etu.SelectedItem.ToString()
                    if($tls_box=$chkChange.Checked)
                    {
                     $recup_group2=Invoke-command -ComputerName $textbox_srv -credential $Cred -ArgumentList $nam_grp -UseSSL -ScriptBlock {param($nam_grp)Get-ADGroupMember -identity  $nam_grp | select SamAccountName}
                    }
                    else{ 
