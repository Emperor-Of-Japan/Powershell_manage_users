...
$textbox_nom_serveur=$Form.FindName("textbox_nom_srv")
 ...
 $Form.FindName("button_connexion").Add_Click( {
    ...
    Invoke-command -ComputerName $textbox_srv -credential $Cred -ScriptBlock {ADD-content -path C:/log.txt -value "----------------------------------------------------------------------------------------------------`r`nscript lancé le $using:date à $using:hour par $using:userName"}
